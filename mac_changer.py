...
def get_arguments():
    ...
    if not options.interface:
        parser.error("[-] Plese enter Interface. use --help for  more info")
    elif not options.new_mac:
        parser.error("[-] Plese enter new mac. use --help for  more info")
    return options

def change_mac(interface, newmac):
    print("[+] Changing MAC for " + interface + " to " + newmac)
